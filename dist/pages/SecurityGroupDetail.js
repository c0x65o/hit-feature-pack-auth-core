...
export function SecurityGroupDetail({ id, onNavigate }) {
    ...
    const packRootScopeSummaryByPack = useMemo(() => {
        ...
        for (const a of actionCatalog) {
            ...
            const m = k.match(/^([a-z][a-z0-9_-]*)\.(read|write|delete)\.scope\.(none|own|location|department|division|all|any)$/);
            if (!m)
                continue;
            const pack = m[1];
            const verb = m[2];
            const raw = m[3];
            const mode = (raw === 'any' ? 'all' : raw);
            if (!present.has(pack))
