...
export async function GET(request, ctx) {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            return NextResponse.json({ error: body?.detail || body?.message || `Failed to fetch user (${res.status})` }, { status: res.status });
        }
...
export async function PUT(request, ctx) {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            return NextResponse.json({ error: err?.detail || err?.message || `Failed to update user (${res.status})` }, { status: res.status });
        }
...
export async function DELETE(request, ctx) {
    ...
    try {
        ...
        if (!res.ok) {
            const err = await res.json().catch(() => ({}));
            return NextResponse.json({ error: err?.detail || err?.message || `Failed to delete user (${res.status})` }, { status: res.status });
        }
...
//# sourceMappingURL=auth-users-id.js.map