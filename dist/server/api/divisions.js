...
export async function GET(request) {
    try {
        ...
        // Divisions are a non-LDD entity: treat this as a simple none/all gate.
        if (mode !== 'all') {
        ...
        const conditions = [];
        if (search) {
            conditions.push(or(like(divisions.name, `%${search}%`), like(divisions.code, `%${search}%`), like(divisions.description, `%${search}%`)));
        }
        ...
        const countQuery = db.select({ count: sql `count(*)` }).from(divisions);
