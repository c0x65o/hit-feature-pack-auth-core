...
function getAuditLogEnabledFromConfig() {
    const auth = HIT_CONFIG?.auth ?? {};
...
export async function tryHandleAuthV2Proxy(opts) {
    ...
    if (p0 === 'audit-log' && m === 'GET') {
        ...
        if (gate)
            return gate;
        if (!getAuditLogEnabledFromConfig())
            return err('Audit log disabled', 403);
        const url = new URL(req.url);
        ...
        const whereParts = [sql `COALESCE(pack_name, '') = 'auth-core'`];
        if (eventType) {
            whereParts.push(sql `COALESCE(event_type, action) = ${eventType}`);
        }
        if (userEmail) {
            whereParts.push(sql `LOWER(COALESCE(target_id, entity_id, actor_id, '')) = ${userEmail}`);
        }
        if (actorEmail) {
    ...
    if (p0 === 'users') {
        ...
        if (m === 'GET' && !p1) {
            ...
            const hasPagingParams = searchParams.has('page') ||
            ...
            const usersListRes = await db.execute(sql `SELECT email, email_verified, two_factor_enabled, locked, role, metadata, created_at, updated_at, last_login
            FROM hit_auth_v2_users
            ORDER BY created_at DESC`);
