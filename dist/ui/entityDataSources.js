...
import { useAuthFeatures, useGroups, useGroup, useGroupMutations, useSegments, useUser, useUserMutations, useUsers, } from '../hooks/useAuthAdmin';
import { useDepartment, useDepartmentMutations, useDepartments, useDivision, useDivisionMutations, useDivisions, useLocation, useLocationMutations, useLocations, useLocationTypes, useUserOrgAssignmentMutations, useUserOrgAssignments, } from '../hooks/useOrgDimensions';
...
function matchesSearch(item, keys, search) {
        ...
        return {
            useList: (args) => {
                ...
                return {
                    ...
                    loading,
                    refetch: refresh,
                    deleteItem: async (id) => mutations.deleteUser(normalizeEmailId(id)),
                };
            },
            useDetail: ({ id }) => {
                ...
                return {
                    record,
                    loading,
                    deleteItem: async (rid) => mutations.deleteUser(normalizeEmailId(rid)),
                };
            },
            useUpsert: ({ id }) => {
            ...
            useListCustomRenderers: () => ({
                created_at: (value) => (value ? formatDateTime(String(value)) : '—'),
                last_login: (value) => (value ? formatDateTime(String(value)) : 'Never'),
                email_verified: (value) => (value ? 'Verified' : 'Pending'),
                two_factor_enabled: (value) => (value ? 'Enabled' : 'Disabled'),
                status: (_, row) => (row.locked ? 'Locked' : 'Active'),
            }),
            renderRowActions: ({ row, onRequestDelete, ui }) => (_jsx("div", { style: { display: 'flex', gap: '8px', justifyContent: 'flex-end' }, children: _jsx(ui.Button, { variant: "ghost", size: "sm", onClick: (e) => {
                        e.stopPropagation();
                        const id = String(row.id || row.email || '');
                        const name = String(row.name || row.email || id);
    ...
    if (entityKey === 'auth.group') {
        ...
        return {
            useList: (args) => {
                ...
                if (search) {
                    items = items.filter((g) => String(g?.name || '').toLowerCase().includes(search));
                }
                const dir = sortOrder === 'asc' ? 1 : -1;
                ...
                return {
                    ...
                    loading,
                    refetch: refresh,
                    deleteItem: async (id) => mutations.deleteGroup(String(id)),
                };
            },
            useDetail: ({ id }) => {
                ...
                return {
                    ...
                    deleteItem: async (rid) => mutations.deleteGroup(String(rid)),
                };
            },
            useUpsert: ({ id }) => {
                ...
                const create = async (payload) => {
                    ...
                    const metadata = dynamicGroupsEnabled
                        ...
                        : undefined;
                    if (dynamicGroupsEnabled && kind === 'dynamic' && !segmentKey)
                        throw new Error('Segment is required for dynamic groups');
            ...
            useListCustomRenderers: () => ({
                created_at: (value) => (value ? formatDate(String(value)) : '—'),
                updated_at: (value) => (value ? formatDate(String(value)) : '—'),
                kind: (value) => (String(value || '').toLowerCase() === 'dynamic' ? 'Dynamic' : 'Static'),
            }),
            renderRowActions: ({ row, onRequestDelete, ui }) => (_jsx("div", { style: { display: 'flex', gap: '8px', justifyContent: 'flex-end' }, children: _jsx(ui.Button, { variant: "ghost", size: "sm", onClick: (e) => {
                        e.stopPropagation();
    ...
    if (entityKey === 'org.location') {
        ...
        return {
            useList: (args) => {
                ...
                const filtered = items.filter((item) => matchesSearch(item, ['name', 'code', 'city', 'state', 'country', 'managerUserKey', 'locationTypeName'], args.search));
                ...
                return {
                    ...
                    loading,
                    refetch: refresh,
                    deleteItem: async (id) => mutations.remove(String(id)),
                };
            },
            useDetail: ({ id }) => {
                ...
                return {
                    ...
                    deleteItem: async (rid) => mutations.remove(String(rid)),
                };
            },
            useUpsert: ({ id }) => {
                const { data: record, loading: detailLoading } = useLocation(id || null);
                const create = async (payload) => mutations.create(payload);
                const update = async (rid, payload) => mutations.update(String(rid), payload);
            ...
            useListCustomRenderers: () => ({
                createdAt: (value) => (value ? formatDate(String(value)) : '—'),
                updatedAt: (value) => (value ? formatDate(String(value)) : '—'),
                isActive: (value) => (value ? 'Active' : 'Inactive'),
                isPrimary: (value) => (value ? 'Yes' : 'No'),
            }),
            renderRowActions: ({ row, onRequestDelete, ui }) => (_jsx("div", { style: { display: 'flex', gap: '8px', justifyContent: 'flex-end' }, children: _jsx(ui.Button, { variant: "ghost", size: "sm", onClick: (e) => {
                        e.stopPropagation();
                        const id = String(row.id || '');
    ...
    if (entityKey === 'org.division') {
        ...
        return {
            useList: (args) => {
                ...
                const filtered = items.filter((item) => matchesSearch(item, ['name', 'code', 'managerUserKey'], args.search));
                const sorted = sortItems(filtered, args.sortBy || 'createdAt', args.sortOrder);
                const page = paginate(sorted, args.page, args.pageSize);
                return {
                    data: { items: page.items, pagination: { total: page.total } },
    ...
    if (entityKey === 'org.department') {
        ...
        return {
            useList: (args) => {
                ...
                const filtered = items.filter((item) => matchesSearch(item, ['name', 'code', 'divisionName', 'managerUserKey'], args.search));
                const sorted = sortItems(filtered, args.sortBy || 'createdAt', args.sortOrder);
                const page = paginate(sorted, args.page, args.pageSize);
                return {
                    data: { items: page.items, pagination: { total: page.total } },
    ...
    if (entityKey === 'org.userOrgAssignment') {
        ...
        return {
            useList: (args) => {
                ...
                const filtered = items.filter((item) => matchesSearch(item, ['userKey', 'divisionName', 'departmentName', 'locationName'], args.search));
                const sorted = sortItems(filtered, args.sortBy || 'createdAt', args.sortOrder);
                const page = paginate(sorted, args.page, args.pageSize);
                return {
                    data: { items: page.items, pagination: { total: page.total } },
            ...
            renderRowActions: ({ row, onRequestDelete, ui }) => (_jsx("div", { style: { display: 'flex', gap: '8px', justifyContent: 'flex-end' }, children: _jsx(ui.Button, { variant: "ghost", size: "sm", onClick: (e) => {
                        e.stopPropagation();
                        const id = String(row.id || '');
                        const name = String(row.userKey || id);
                        onRequestDelete({ id, name });
                    }, children: _jsx(Trash2, { size: 16, style: { color: 'var(--hit-error, #ef4444)' } }) }) })),
        };
    }
...
//# sourceMappingURL=entityDataSources.js.map