...
export function OAuthButtons({ providers }) {
    ...
    if (availableProviders.length === 0)
        return null;
    return (_jsxs("div", { className: "space-y-3", children: [_jsxs("div", { className: "flex items-center gap-3 my-6", children: [_jsx("div", { className: "flex-1 border-t border-[var(--hit-border)]" }), _jsx("span", { className: "text-sm text-[var(--hit-muted-foreground)]", children: "or continue with" }), _jsx("div", { className: "flex-1 border-t border-[var(--hit-border)]" })] }), _jsx("div", { className: `grid gap-3 ${availableProviders.length > 2 ? 'grid-cols-1' : 'grid-cols-' + availableProviders.length}`, children: availableProviders.map((provider) => (_jsxs("button", { type: "button", onClick: () => initiateOAuth(provider.id), className: `
