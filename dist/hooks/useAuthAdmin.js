...
export function useUserMutations() {
    ...
    const uploadProfilePictureBase64 = async (email, base64DataUrl) => {
        setLoading(true);
        setError(null);
        try {
...
export function useUserEffectivePermissions(userEmail) {
    const [data, setData] = useState(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);
    const refresh = useCallback(async () => {
        ...
        try {
            setLoading(true);
            setError(null);
            const result = await fetchWithAuth(`/admin/permissions/users/${encodeURIComponent(userEmail)}/effective`);
            setData(result);
        }
        catch (e) {
            setError(e);
...
export function usePermissionSetMutations() {
    ...
    const updatePermissionSet = useCallback(async (id, data) => {
        setLoading(true);
        setError(null);
        try {
            return await fetchWithAuth(`/admin/permissions/sets/${id}`, {
                method: 'PATCH',
                body: JSON.stringify(data),
            });
        }
