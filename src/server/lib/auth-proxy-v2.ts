...
function normalizeSessionRow(s: any): Record<string, unknown> {
  return {
    id: String(s?.id || ''),
...
export async function checkActionPermissionV2(
  ...
  const inferredRole: 'admin' | 'user' = u.roles.includes('admin') ? 'admin' : 'user';
  ...
  let role: 'admin' | 'user' = inferredRole;
  if (role !== 'admin') {
    try {
      ...
      if (groupIds.length > 0) {
        where = sql`${where} OR (a.principal_type = 'group' AND a.principal_id IN (${sql.join(
  ...
  if (p0 === 'admin' && (p1 || '').trim() === 'permissions' && (pathSegments[2] || '').trim() === 'sets') {
    ...
    if (sub === 'actions' && tail === 'blocks' && m === 'POST') {
      ...
      try {
        ...
        const msg = String(e?.message || e || '').toLowerCase();
        if (msg.includes('duplicate') || msg.includes('unique')) return err('Action block already exists', 400);
    ...
    if (sub === 'actions' && tail === 'blocks' && tail2 && m === 'DELETE') {
      await db.execute(
        sql`DELETE FROM hit_auth_v2_permission_seed_keys WHERE id = ${tail2}::uuid AND permission_set_id = ${psId}::uuid AND kind = 'block_action'`
      );
      return json({}, { status: 204 });
    }
