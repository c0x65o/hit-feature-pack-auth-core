...
export interface AclResolutionContext {
  // User's principals (their ID, groups they belong to, their roles)
  userPrincipals: {
    userId: string;
    groupIds: string[];
    roles: string[];
  ...
  // All ACL entries for the resource
  entries: AclEntry[];
  
  // Permission configuration (for hierarchical mode)
...
export function resolveEffectivePermissions(
  ...
  for (const entry of entries) {
    ...
    if (matches && Array.isArray(entry.permissions)) {
      entry.permissions.forEach((perm: string) => permissionSet.add(perm));
    }
  }
...
export function getEffectiveLevel(
  ...
  for (const level of sortedPermissions) {
    if (level.includes && level.includes.length > 0) {
      ...
      const hasAllPermissions = level.includes.every((perm: string) => effectivePermissions.includes(perm));
  ...
  for (const level of reverseSorted) {
    if (level.includes && level.includes.some((perm: string) => effectivePermissions.includes(perm))) {
      return level;
    }
    if (effectivePermissions.includes(level.key)) {
      return level;
...
export function principalMatches(
  ...
  principalId: string,
  userPrincipals: AclResolutionContext['userPrincipals']
): boolean {
